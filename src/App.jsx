...
const initialTexts = [
  { id: 1, title: "Sample Text 1", content: "This is the first sample text. It contains a quote.", quotes: [{ id: 1, text: "It contains a quote", start: 35, end: 53 }] },
  { id: 2, title: "Sample Text 2", content: "Here's another sample text with a different quote.", quotes: [{ id: 2, text: "with a different quote", start: 33, end: 55 }] },
];

function App() {
  const [texts] = useState(initialTexts);
      ...
      <div className="container mx-auto p-4">
        <nav className="mb-4">
          <ul className="flex space-x-4">
            <li><Link to="/" className="text-blue-500 hover:text-blue-700">Texts</Link></li>
            <li><Link to="/quotes" className="text-blue-500 hover:text-blue-700">Quotes</Link></li>
          </ul>
        </nav>
      ...
      <ul>
        {texts.map(text => (
          <li key={text.id} className="mb-2">
            <Link to={`/text/${text.id}`} className="text-blue-500 hover:text-blue-700">{text.title}</Link>
          </li>
        ))}
      </ul>
    </div>
        ...
        {allQuotes.map(quote => (
          <li key={quote.id} className="mb-2">
            <span
              ...
              onClick={() => navigate(`/text/${quote.textId}`, { state: { highlightQuote: quote.id } })}
            >
